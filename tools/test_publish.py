...
class PublishTests(unittest.TestCase):
    ...
    def test_config_settings_and_defaults(self):
        ...
        self.assertEqual(publish.read_project_id(self.env), 398695)
    ...
    def test_configured_preview_and_upload(self):
        ...
        output = StringIO()
        with patch("publish.api_json") as api, patch("publish.build_archive") as build, redirect_stdout(output):
            self.assertEqual(publish.main(["--version", "2.0.1", "--env-file", str(self.env), "--dry-run"]), 0)
        ...
        with patch("publish.api_json", side_effect=[[{"name": "1.60.2", "id": 99}], {"id": 456}]) as api, \
                patch("publish.build_archive", return_value=archive), redirect_stdout(StringIO()):
            self.assertEqual(publish.main(["--version", "2.0.1", "--env-file", str(self.env)]), 0)
    ...
    def test_missing_second_version_prevents_build_and_upload(self):
        ...
        with patch("publish.api_json", return_value=[{"name": "1.60.1", "id": 42}]) as api, \
                patch("publish.build_archive") as build, redirect_stdout(StringIO()), redirect_stderr(StringIO()):
            self.assertEqual(publish.main(["--version", "2.0.1", "--env-file", str(self.env)]), 1)
    ...
    def test_dry_run_never_builds_reads_credentials_or_calls_api(self):
        with patch("publish.read_token") as token, patch("publish.api_json") as api, \
                patch("publish.build_archive") as build, redirect_stdout(StringIO()):
            self.assertEqual(publish.main(["--version", "2.0.1", "--dry-run", "--env-file", str(self.env)]), 0)
    ...
    def test_package_only_builds_without_token_or_network(self):
        output = StringIO()
        with patch("publish.read_token") as token, patch("publish.api_json") as api, \
                patch("publish.build_archive", return_value=self.root / "FishMaster-2.0.1.zip") as build, \
                redirect_stdout(output):
            self.assertEqual(publish.main(["--version", "2.0.1", "--package-only", "--env-file", str(self.env)]), 0)
        token.assert_not_called()
        api.assert_not_called()
    ...
    def test_package_only_and_dry_run_are_mutually_exclusive(self):
        with redirect_stderr(StringIO()), self.assertRaises(SystemExit):
            publish.main(["--version", "2.0.1", "--package-only", "--dry-run"])

    def test_real_payload_excludes_env_and_publishing_tools(self):
        included, _ = validate()
        payload = [path.relative_to(publish.ROOT).as_posix() for path in included]
        self.assertIn("FishMaster.toc", payload)
        self.assertFalse(any(".env" in p or p.startswith(("tools/", "tests/", ".github/")) for p in payload))

    def test_archive_checksum_and_source_version_preserved(self):
        archive = self.root / "FishMaster-2.0.1.zip"
        archive.write_bytes(b"test archive")
        with patch("publish.build", return_value=(archive, "release")):
            self.assertEqual(publish.build_archive("2.0.1"), archive)
        self.assertEqual(archive.with_suffix('.zip.sha256').read_text(),
                         hashlib.sha256(archive.read_bytes()).hexdigest() + "  " + archive.name + "\n")

    def test_upload_preserves_release_channels(self):
        archive = self.root / "release.zip"
        archive.write_bytes(b"test archive")
        for version, kind in (("2.0.0-forever", "release"),
                              ("2.0.1-alpha.1", "alpha"), ("2.0.1-beta.1", "beta")):
            with patch("publish.api_json", return_value={"id": 123}) as api:
                publish.upload(archive, version, "notes", [42], "private-token")
                self.assertIn(('"releaseType": "' + kind + '"').encode(), api.call_args.args[2])

    def test_invalid_version_prevents_network_and_build(self):
        with patch("publish.api_json") as api, patch("publish.build_archive") as build, \
                redirect_stdout(StringIO()), redirect_stderr(StringIO()):
            self.assertEqual(publish.main(["--version", "../../bad", "--env-file", str(self.env)]), 1)
        api.assert_not_called()
        build.assert_not_called()
    ...
    def test_version_lookup_failure_prevents_build_and_upload(self):
        with patch("publish.read_token", return_value="private-token"), \
                ...
                patch("publish.build_archive") as build, \
                redirect_stdout(StringIO()), redirect_stderr(StringIO()):
            self.assertEqual(publish.main(["--version", "2.0.1", "--env-file", str(self.env)]), 1)
            self.assertEqual(api.call_count, 1)
            build.assert_not_called()
    ...
    def test_complete_publish_flow_with_mock_api(self):
        ...
        with patch("publish.api_json", side_effect=[
                ...
                patch("publish.build_archive", return_value=archive), \
                redirect_stdout(output):
            self.assertEqual(publish.main(["--version", "2.0.1", "--env-file", str(self.env)]), 0)
        self.assertEqual(api.call_count, 2)
        path, token, body, content_type = api.call_args.args
        self.assertEqual(path, "/projects/398695/upload-file")
        ...
        self.assertIn("/fishmaster/files/12345", output.getvalue())
        ...
        for response in ({}, {"id": True}, {"id": 0}, {"id": "123"}):
            with patch("publish.api_json", return_value=response) as api:
                with self.assertRaisesRegex(publish.PublishError, "Check the author dashboard"):
                    publish.upload(archive, "2.0.1", "notes", [42], "private-token")
                self.assertEqual(api.call_count, 1)
