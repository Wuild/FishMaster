#!/usr/bin/env python3
"""Publish FishMaster to CurseForge using the upload token in a local .env file."""
...
from package import build, validate, release_metadata

ROOT = Path(__file__).resolve().parents[1]
API = "https://wow.curseforge.com/api"
DEFAULT_PROJECT_ID = 398695
...
def api_json(path, token, data=None, content_type=None):
    headers = {"X-Api-Token": token, "Accept": "application/json",
               "User-Agent": "FishMaster-Publisher/1.0"}
    if content_type:
...
def upload(archive, version, notes, version_ids, token, project_id=DEFAULT_PROJECT_ID):
    metadata = {"displayName": f"FishMaster {version}", "changelog": notes,
                "changelogType": "markdown", "gameVersions": version_ids,
                "releaseType": release_metadata(version)[1]}
    ...
    if project_id == DEFAULT_PROJECT_ID:
        return f"https://www.curseforge.com/wow/addons/fishmaster/files/{file_id}"
    return f"https://legacy.curseforge.com/projects/{project_id}/files/{file_id}"


def build_archive(version):
    archive, _ = build(version)
...
def main(argv=None):
    ...
    try:
        ...
        notes = (args.changelog.read_text(encoding="utf-8-sig") if args.changelog
                 else f"FishMaster {args.version} for WoW Forever {game_version}.")
        version, _ = release_metadata(args.version)
        payload, _ = validate()
        print(f"FishMaster {version}: {len(payload)} files; CurseForge project {project_id}; WoW {game_version}")
        if args.dry_run:
            print(notes)
